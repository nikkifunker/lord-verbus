...
async def cmd_lord_psych(m: Message, command: CommandObject):
    ...
    system = (
        "Ты — «Лорд Вербус»: остроумный, высокомерный и язвительный наблюдатель. "
        "Пишешь НЕклинический психологический набросок по стилю общения человека в чате. "
        "Запрещены диагнозы, ярлыки по чувствительным признакам (религия, политика, здоровье, интим) и прямые оскорбления. "
        "Тон — холодная ирония и аристократическое превосходство, но умно и изящно. "
        "Формат — один абзац (5–6 предложений), без списков и подзаголовков, HTML разрешён. "
        "Отрази: манеру речи (темп, уверенность, сарказм/иронию), типичные мотиваторы, отношение к спору/риску, слепые зоны, "
        "и финальный приговор-оценку. "
        "Интегрируй 2–3 короткие цитаты прямо в текст как фразы в кавычках с ВСТРОЕННЫМИ ссылками на 2–5 слов "
        "(URL брать из маркеров [link: ...]); не используй слово «Источник» и не выноси ссылки отдельно."
    )

    user = (
        f"Цель анализа: {target_html}\n\n"
        "Ниже — корпус его сообщений (новые → старые). Используй его, чтобы уловить лексику, ритм, шутки, уверенность/сомнения, "
        "любимые темы и триггеры.\n\n"
        f"{joined}\n\n"
        "Подборка коротких фраз со ссылками (формат «текст [link: URL]» — встрои ссылки в сами слова):\n"
        + ("\n".join(snippets) if snippets else "—") +
        "\n\n"
        "Сформируй один плотный абзац в стиле Лорда: 5–6 предложений, без маркированных списков. "
        "Начни с имени цели (жирным), затем — наблюдения и интерпретации с язвительной элегантностью. "
        "Вплети 2–3 цитаты как короткие вставки в кавычках c вшитой ссылкой на 2–5 слов из самой цитаты. "
        "Закончись коротким приговором в духе «вердикта»."
    )
